        ...
        {CTAS.map((_, index) => (
          <div key={index} className="flex flex-col md:flex-row gap-xs">
            <div className="flex-1">
              <h3 className={clsx("block font-infra transform-none text-base md:text-xl text-red font-bold leading-tight", isRTL(lang) && "text-right")}>
                {t(`participate:cta${index + 1}-title`)}
              </h3>
            </div>
            <div
              className={clsx("block flex-1 text-red text-xs md:text-lg font-infra", isRTL(lang) && "text-right")}
